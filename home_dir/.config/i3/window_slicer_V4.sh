...
# getting cursor coordinates and select window under mouse cursor
eval $(xdotool getmouselocation --shell mousedown 1 mouseup 1)
...
slicing(){
    change=$(( $1 / 2 - ($2 - $3) ))
    # i3-msg split "$5" && alacritty & disown && waitloop
    i3-msg split "$5" && i3-msg open
    if [ "$change" -gt 0 ]; then
